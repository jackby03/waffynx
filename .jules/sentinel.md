...
## 2024-05-24 - DOM XSS in Dashboard Real-Time Logs
**Vulnerability:** The management API dashboard (`cmd/waf-api/ui/index.html`) concatenated attacker-controlled data (such as request path, HTTP method, attack type, and IP address from `recentAttacks`) and dynamic configuration values directly into `element.innerHTML` using template literals, leading to DOM-based Cross-Site Scripting (XSS).
**Learning:** Even internal management interfaces require strict output encoding. When rendering dynamic data into the DOM, relying solely on JavaScript template literals assigned to `innerHTML` is inherently unsafe because the browser parses the string as HTML, executing any embedded malicious scripts. This is especially critical for a WAF dashboard that displays malicious payloads sent by attackers.
**Prevention:** Always use safe DOM manipulation methods like `.textContent` or properly encode dynamic data before inserting it into HTML contexts. Implemented a robust `escapeHTML` helper function and applied it to all data points interpolated into HTML strings to neutralize potential XSS vectors.
