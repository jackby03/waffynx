...
## 2026-09-15 - Missing CORS Middleware on All Authenticated Endpoints
**Vulnerability:** The CORS middleware was previously applied on a per-route basis via a `withCORS` wrapper, and an explicit `OPTIONS` route was only registered for `/api/v1/auth/login`. This meant preflight `OPTIONS` requests for all other authenticated endpoints (e.g., `/api/v1/config`, `/api/v1/firewall/rules`) were not matching any routes and were implicitly rejected by the router.
**Learning:** In Go 1.22's `http.ServeMux`, method-specific route registration means preflight `OPTIONS` requests won't match automatically if only `GET`, `POST`, etc., are registered. Applying CORS middleware to individual routes without explicitly handling `OPTIONS` for all of them leads to broken cross-origin requests for those endpoints.
**Prevention:** Apply CORS middleware globally at the top level of the handler chain (wrapping the entire `ServeMux`) rather than on individual routes. This ensures all `OPTIONS` requests are intercepted, validated, and resolved by the middleware before the router attempts to match them against method-specific routes.
