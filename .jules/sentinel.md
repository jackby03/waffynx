...
## 2024-10-25 - Global CORS Preflight Handling in Go 1.22
**Vulnerability:** In Go 1.22's `http.ServeMux`, because method-specific routes (e.g., `POST /api/...`) are used, preflight `OPTIONS` requests would return 404 (or 405) unless specifically registered. A previous workaround registered an unauthenticated `OPTIONS` route for a specific endpoint but left others (like `/api/v1/events` for SSE) without CORS preflight support, and registering unauthenticated routes can be dangerous if the middleware isn't strictly enforcing logic.
**Learning:** Do not blindly map business-logic handlers to unauthenticated `OPTIONS` routes to resolve CORS. This creates a security risk where unintended logic might execute on preflight requests.
**Prevention:** Instead, use dedicated CORS middleware that accepts an `http.Handler` and wraps the *global* `mux` instance at the server level (e.g., inside the `Handler()` method). This safely intercepts and resolves all preflight requests before they ever reach the router, ensuring consistent and secure CORS policy enforcement across all endpoints.
